...
/**
 * Orquesta la cortina (veil) de transición entre secciones, sincronizada con
 * el router. Reemplaza el `go()` imperativo del sitio original:
 *
 *   click → NavigationStart → la cortina sube (cubre la sección actual y la
 *   carga del chunk lazy) → `pageTransitionGuard` espera a que la cortina cubra
 *   del todo (`coverComplete`) antes de activar la ruta → la nueva sección se
 *   monta OCULTA detrás de la cortina → NavigationEnd → la cortina baja y revela.
 *
...
export class PageTransition {
  ...
  private uncover(): void {
    ...
    v.style.transform = 'scaleY(0)';
    m.style.opacity = '0';
    m.style.transform = 'scale(1.12) rotate(8deg)';
    setTimeout(() => {
