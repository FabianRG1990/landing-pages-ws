 ...
 * propia lib feature y se carga lazy (default export = rutas). `inicio` es la
 * raíz canónica y se sirve en `/` (la landing aterriza ahí, sin redirección).
 * `pageTransitionGuard` retiene la activación hasta que la cortina cubre, para
 * que el cambio de sección ocurra oculto detrás de la animación.
 */
